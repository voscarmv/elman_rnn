...
*/
			...
			success = netpredict(net, data, m, train + 1);
			...
			if(!success && data->dataset[m + train][1] == 1)
				pred = -1;
/*
			printf("%d.%d. qlty %.2f, netp %.4f, "
				"randp %.4f, sxs %d\n",
				(m+1), (p+1), (bench-best)/bench, (double) netp/((m*31)+(p+1)),
				(double) randp/((m*31)+(p+1)), success);
*/

			printf("%d.%d. act %.0f, pred %d, sxs %d, qlty %.0f\n",
				(m+1), (p+1), data->dataset[m + train][1],
				pred, success, best);
