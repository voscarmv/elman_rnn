...
/**/
	...
	;
	for(m = 0; m < 4000; m ++){
		data2 = datacpy(data, input + output, m, 10);
		best = fitness(net, data2);
	//	printf("Benchmark fitness: %f\n", best);

		for(i = 0; i < 10000 && best > 0; i ++){
	...
	//			printf("%d: Found better fitness: %f\n", i, better);
		...
		if(best > 0)
			continue;
		if(netpredict(net, data, m, 11))
			netp ++;
		if(onepredict(net, data, m, 11))
			onep ++;
		if(zeropredict(net, data, m, 11))
			zerop ++;
		if(randpredict(net, data, m, 11))
			randp ++;
		if(kuperinpredict(net, data, m, 11))
			kuperinp ++;
		printf("%d. %d. netp %d, onep %d, zerop %d, randp %d, kuperinp %d\n",
			q ++, m, netp, onep, zerop, randp, kuperinp);
		fflush(NULL);
	}
...
int netpredict(ElmanPtr net, DatasetPtr data, int from, int length){
	...
	if(sigmoid(net->neurons[OUTPUT][0]) == data->dataset[from + i - 1][1])
		return 1;
	else
		return 0;
}
...
int onepredict(ElmanPtr net, DatasetPtr data, int from, int length){
	...
	if(data->dataset[from + length - 1][1] == 1)
		return 1;
	else
		return 0;
}
...
int randpredict(ElmanPtr net, DatasetPtr data, int from, int length){
	...
	if(rand() % 2)
		...
		result = 1;
	if(data->dataset[from + length - 1][1] == result)
		return 1;
	else
		return 0;
}
...
*/
	if(data->dataset[from + length - 1][1] == data->dataset[from + length - 2][1])
		return 1;
	else
		return 0;
}
