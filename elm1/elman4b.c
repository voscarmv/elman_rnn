...
/**/
		...
		for(i = 0; i < population; i ++){
			...
			randnet(nets[i]);
//			fitness(nets[i], data);
			printf("Fitness %d: %f\n", i, fitness(nets[i], data));
//			printf("%d/%d, ", i + 1, population);
			fflush(NULL);
		}
		for(j = 0; j < epochs; j ++){
			qsort(nets, population, sizeof(nets[0]), comparenets);
			for(i = 0; i < chosen; i ++)
				printf("Epoch %d/%d: %f\n", j + 1, epochs, nets[i]->fitness);
			fflush(NULL);
			sprintf(fname, "epoch%d.dat", j);
			fprintnetio(fname, nets, data, chosen);
			printf("Start: ");
			fflush(NULL);
...
double fitness(ElmanPtr net, DatasetPtr data){
	...
	for(i = 0; i < data->dataset_l; i ++){
		...
		for(j = 0; j < net->lengths[OUTPUT]; j ++){
			...
			expected = data->dataset[i][net->lengths[INPUT] - 1 + j];
